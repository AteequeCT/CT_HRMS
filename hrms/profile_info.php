...
<body>
                        ...
                        <div class="card profile-card">
                            <div class="profile-header">
                                <button id="editProfileBtn" class="btn btn-sm btn-outline-light float-end" title="Edit Profile">
                                    ...
                                    <!-- Hidden input to send current image path -->
                                    <input type="hidden" name="current_img_path" value="<?php echo $emp['profile_image'] ?? ''; ?>">
                                ...
                                <div class="row g-4">
                                    ...
                                    ?>
                                        <div class="col-md-6">
                                            ...
                                            <p class="profile-info-value" data-key="<?php echo $label; ?>"><?php echo htmlspecialchars($value); ?></p>
                                        </div>
                                    <?php endforeach; ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
    // Remove the save button completely from your HTML
    // Or keep it hidden if needed for other purposes
    
    // Image upload handler - triggers immediately on file selection
    ...
    function uploadProfileImage(file) {
        ...
        // Add current image path if exists
        const currentImgPath = document.querySelector('input[name="current_img_path"]').value;
        if (currentImgPath) {
            formData.append('current_img_path', currentImgPath);
        }
            ...
            success: function(response) {
                ...
                if (response.trim() === '1') {
                    ...
                    $.notify({
                        ...
                        message: 'Failed to update profile image. Server said: ' + response,
                    }, {
                        type: 'danger',
                        placement: { from: "top", align: "right" },
                        timer: 3000,
                    ...
                    document.getElementById('profileImageDisplay').src = document.querySelector('input[name="current_img_path"]').value;
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error:', status, error, xhr.responseText);
    ...
    // Keep your existing edit functionality for other fields if needed
    const editBtn = document.getElementById('editProfileBtn');
    if (editBtn) {
        editBtn.addEventListener('click', function() {
            // Your existing edit field code here
        });
    }
});
    </script>
